...
PID_FILE="$HOME"/.news-"$WORKER_NAME".pid
CONFIG_FILE="$PRGDIR"/conf/"$WORKER_NAME".properties
...
if [ "$1" = "start" ] ; then
  ...
  LOG4J_CONFIG="$PRGDIR"/conf/log4j.properties
  WORKER_EXEC="$JAVA_HOME"'/bin/java '$JAVA_OPTS'-Dlog4j.configuration=file:'$LOG4J_CONFIG' -classpath '\""$CLASSPATH"\"' -Dconfig.file='$CONFIG_FILE' '"$LAUNCHER_CLASS"
  ...
  echo "Usage: worker-ctrl.sh ( command )"
  echo "commands:"
